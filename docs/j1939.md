...
```bash
# In one terminal - start the server, auto-connect, and enable J1939
canviz serve --interface slcan --channel COM3 --bitrate 250000 --j1939

# In another terminal - check J1939 status
canviz j1939 status
```

For headless / SSH workflows:

```bash
canviz serve --interface slcan --channel COM3 --bitrate 250000 --headless --j1939 &
canviz j1939 status
```
